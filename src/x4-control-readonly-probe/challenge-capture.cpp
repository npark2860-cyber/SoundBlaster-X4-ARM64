...
struct Logger
{
    std::ofstream file{"X4_CTCDC_CHALLENGE_REPORT.txt", std::ios::binary | std::ios::trunc};
...
{
    Logger log;
    log.line("Sound Blaster X4 CTCDC challenge-capture probe");
    log.line("Diagnostic only: sends readiness query and, only if needed, CTCDC unlock hello. No unlock reply, SW_MODE1, or feature SET is sent.");

    if (argc > 2)
    {
        log.line("Usage: x4-ctcdc-challenge-capture.exe [COMx]");
        return 2;
    }
    ...
    else
    {
        ...
        else
        {
            ...
            if (find_max_payload_response(response, maxPayload))
            {
                ...
                log.line(text.str());
                log.line("Fast path is ready; no unlock hello was needed.");
            }
            else
            {
                log.line("No valid command-0x03 response. Entering CTCDC challenge observation path.");
                ...
                else
                {
                    ...
                    if (starts_with_ascii(unlockResponse, "whoareyou"))
                    {
                        ...
                        log.line("STOP: challenge captured; no cryptographic unlock reply is generated or sent.");
                    }
                    else if (starts_with_ascii(unlockResponse, "NotYet\r\n"))
                    {
                        log.line("Device replied NotYet; CTCDC normally retries after a delay. Probe stops here.");
                    }
                    else if (starts_with_ascii(unlockResponse, "Unknown command\r\n"))
                    {
                        log.line("Device replied Unknown command. Probe stops here.");
                    }
                    else
                    {
    ...
    CloseHandle(handle);
    log.line("Done. Upload X4_CTCDC_CHALLENGE_REPORT.txt for analysis.");
    return result;
}
