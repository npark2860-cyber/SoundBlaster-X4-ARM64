...
std::string hex_u16(std::uint16_t value)
{
    ...
    out << "0x" << std::uppercase << std::hex << std::setfill('0') << std::setw(4)
...
{
    std::ostringstream out;
    out << "0x" << std::uppercase << std::hex << std::setfill('0') << std::setw(8)
        << value;
    return out.str();
}
...
void log_ack_if_present(Logger& log, std::vector<Frame> const& frames, std::uint8_t associatedCommand)
{
    for (auto const& frame : frames)
    {
        if (frame.command == 0x02 && frame.payload.size() >= 2 && frame.payload[0] == associatedCommand)
        {
            log.line("ACK for command 0x" + byte_hex(associatedCommand) +
                ": status=0x" + byte_hex(frame.payload[1]) + " (" + ack_status_name(frame.payload[1]) + ")");
        }
    }
}
...
void redirect_log(Logger& log)
{
    ...
    log.file.open("X4_MIXER_DRILLDOWN_REPORT.txt", std::ios::binary | std::ios::trunc);
}
}
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    HANDLE const handle = CreateFileW(normalize_port(port).c_str(), GENERIC_READ | GENERIC_WRITE,
    ...
    log.line("=== Session gate ===");
    {
        ...
        auto const maxPayload = u16le(frame->payload, 0);
        log.line("VALID Maximum Payload Size: " + std::to_string(maxPayload));
    }
    {
    ...
    log.line("=== Malcolm sub-feature support (read-only) ===");
    {
        ...
        if (auto const* frame = find_command(frames, 0x10); frame && frame->payload.size() >= 8)
        {
            auto const featureMask = u32le(frame->payload, 0);
            auto const unavailableMask = u32le(frame->payload, 4);
            log.line("FeatureMask: " + hex_u32(featureMask));
            log.line("UnavailableMask: " + hex_u32(unavailableMask));
        }
        else
        {
    ...
    std::vector<MixerDescriptor> descriptors;
    {
        ...
        log.line("Total=" + std::to_string(total) + " Count=" + std::to_string(count) +
            " AdditionalPacket=" + std::to_string(additionalPacket) +
            " AudioControlIndex=" + std::to_string(baseIndex));

        if (count > 32 || frame->payload.size() < static_cast<std::size_t>(4 + count * 2))
        {
        ...
        {
            auto const raw = u16le(frame->payload, 4 + static_cast<std::size_t>(i) * 2);
            MixerDescriptor descriptor{};
            descriptor.index = static_cast<std::uint8_t>(baseIndex + i);
            descriptor.raw = raw;
            descriptor.type = static_cast<std::uint8_t>(raw & 0x003F);
            descriptor.hasVolume = (raw & 0x0040) != 0;
            descriptor.hasMute = (raw & 0x0080) != 0;
            descriptor.isSource = (raw & 0x0100) != 0;
            descriptor.isRecording = (raw & 0x0200) != 0;
            descriptor.hasCustomName = (raw & 0x0400) != 0;
            descriptor.isDbLevel = (raw & 0x0800) != 0;
            descriptor.channelNibble = static_cast<std::uint8_t>((raw >> 12) & 0x0F);
            descriptors.push_back(descriptor);

            log.line("index=" + std::to_string(descriptor.index) +
                " raw=" + hex_u16(raw) +
                " type=" + audio_control_type_name(descriptor.type) +
                " volume=" + (descriptor.hasVolume ? "Y" : "N") +
                " mute=" + (descriptor.hasMute ? "Y" : "N") +
                " source=" + (descriptor.isSource ? "Y" : "N") +
                " recording=" + (descriptor.isRecording ? "Y" : "N") +
                " customName=" + (descriptor.hasCustomName ? "Y" : "N") +
                " dB=" + (descriptor.isDbLevel ? "Y" : "N") +
                " channelNibble=" + std::to_string(descriptor.channelNibble));
        }

        auto const used = static_cast<std::size_t>(4 + count * 2);
        if (frame->payload.size() > used)
        {
            std::vector<std::uint8_t> trailing(frame->payload.begin() + used, frame->payload.end());
            log.line("Trailing payload bytes after Count descriptors (not interpreted): " + hex_bytes(trailing));
        }
    }
    ...
    log.line("=== AudioControl level ranges (read-only) ===");
    {
        ...
        if (auto const* frame = find_command(frames, 0x22); frame && frame->payload.size() >= 2)
        {
            ...
            if (frame->payload.size() >= needed)
            {
                for (std::uint8_t i = 0; i < count; ++i)
                {
                    ...
                    log.line("range index=" + std::to_string(index) +
                        " maxRaw=" + hex_u16(maxValue) + " maxSigned=" + std::to_string(static_cast<std::int16_t>(maxValue)) +
                        " minRaw=" + hex_u16(minValue) + " minSigned=" + std::to_string(static_cast<std::int16_t>(minValue)) +
                        " stepRaw=" + hex_u16(stepValue) + " stepSigned=" + std::to_string(static_cast<std::int16_t>(stepValue)));
                }
            }
            else
            {
    ...
    for (auto const& descriptor : descriptors)
    {
        if (!descriptor.hasVolume)
            continue;
        Query const q{"Mixer", "AudioLevel GET index=" + std::to_string(descriptor.index),
            0x23, {0x01, descriptor.index}};
        auto const frames = transact_capture(handle, q, log, 1500, 80);
        if (auto const* frame = find_command(frames, 0x23); frame && frame->payload.size() >= 3)
        {
            auto const index = frame->payload[0];
            auto const current = u16le(frame->payload, 1);
            log.line("level index=" + std::to_string(index) +
                " raw=" + hex_u16(current) +
        ...
        else
        {
            ...
            log.line("No parseable AudioLevel GET payload for index " + std::to_string(descriptor.index));
        }
    }
    ...
    for (auto const& descriptor : descriptors)
    {
        if (!descriptor.hasMute)
            continue;
        Query const q{"Mixer", "AudioMute GET index=" + std::to_string(descriptor.index),
            0x24, {0x01, descriptor.index}};
        auto const frames = transact_capture(handle, q, log, 1500, 80);
        if (auto const* frame = find_command(frames, 0x24); frame && frame->payload.size() >= 2)
        {
        ...
        else
        {
            ...
            log.line("No parseable AudioMute GET payload for index " + std::to_string(descriptor.index));
        }
    }
