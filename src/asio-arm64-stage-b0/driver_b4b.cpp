...
#error Stage B4B must be built for native Windows ARM64, not ARM64EC.
#endif
...
public:
    ...
    ASIOBool init(void* sysHandle) override {
        ...
        if (worker_thread_ || engine_.running()) {
            ...
            if (stop_result != ASE_OK && worker_thread_) {
                strcpy_s(last_error_, "B4B init rejected: existing worker could not be joined safely");
                return ASIOFalse;
            }
        }
        ...
        if (!preflight.device_found) {
            strcpy_s(last_error_, "B4B init FAILED: X4 msft_wave not found");
            return ASIOFalse;
        }
        if (!preflight.filter_opened) {
            sprintf_s(last_error_, sizeof(last_error_),
                      "B4B init FAILED: filter open Win32=%lu", preflight.open_error);
            return ASIOFalse;
        }
        if (!preflight.local_ok || !preflight.global_ok) {
            sprintf_s(last_error_, sizeof(last_error_),
                      "B4B init INDETERMINATE: C ok=%d G ok=%d; KsCreatePin SKIPPED",
        ...
        if (busy) {
            sprintf_s(last_error_, sizeof(last_error_),
                      "B4B init BUSY: C %lu/%lu G %lu/%lu; KsCreatePin SKIPPED",
                      preflight.local_current, preflight.local_possible,
                      preflight.global_current, preflight.global_possible);
            return ASIOFalse;
        }
        ...
        sprintf_s(last_error_, sizeof(last_error_),
                  "B4B init FREE: C %lu/%lu G %lu/%lu; host query contract ready",
                  preflight.local_current, preflight.local_possible,
                  preflight.global_current, preflight.global_possible);
        return ASIOTrue;
    }
    ...
    long getDriverVersion() override { return 106; }
    ...
    ASIOError start() override {
        if (!initialized_free_ || !buffers_created_ || !engine_.prepared()) {
            strcpy_s(last_error_, "B4B start rejected: ASIO/WaveRT buffers not prepared");
            return ASE_InvalidMode;
        }
        if (worker_thread_ || engine_.running()) {
            strcpy_s(last_error_, "B4B start rejected: worker/RUN already active");
            return ASE_InvalidMode;
        }
        ...
        if (!stop_event_) {
            sprintf_s(last_error_, sizeof(last_error_),
                      "B4B start FAILED: stop event Win32=%lu", GetLastError());
            return ASE_NoMemory;
        }
        ...
        if (!worker_thread_) {
            ...
            worker_thread_id_ = 0;
            sprintf_s(last_error_, sizeof(last_error_),
                      "B4B start FAILED: CreateThread Win32=%lu", error);
            return ASE_NoMemory;
        }

        sprintf_s(last_error_, sizeof(last_error_),
                  "B4B start OK: workerThreadId=%lu; host query position reset=0",
                  worker_thread_id_);
        return ASE_OK;
    }
    ...
    ASIOError stop() override {
        ...
        if (worker_thread_) {
            ...
            if (wait != WAIT_OBJECT_0) {
                worker_joined = false;
                sprintf_s(last_error_, sizeof(last_error_),
                          "B4B stop FAILED: worker join wait=%lu; hardware teardown WITHHELD",
                          wait);
                return ASE_HWMalfunction;
            }
        ...
        if (!state_ok || worker_failed || index_errors || copy_errors ||
            ...
            sprintf_s(last_error_, sizeof(last_error_),
                      "B4B stop FAIL joined=%s workerFail=%ld cb=%ld idxErr=%ld copyErr=%ld pktErr=%lu posErr=%lu",
        ...
        sprintf_s(last_error_, sizeof(last_error_),
                  "B4B stop OK workerJoined=YES notif=%lu cb=%ld dmaWrites=%lu dmaFrames=%lu",
                  stats.notifications,
                  callback_count,
    ...
    ASIOError createBuffers(
        ...
        if (!initialized_free_) {
            strcpy_s(last_error_, "B4B createBuffers rejected: init not FREE");
            return ASE_InvalidMode;
        }
        if (!bufferInfos || !callbacks || !callbacks->bufferSwitch) {
            strcpy_s(last_error_, "B4B createBuffers requires bufferInfos + bufferSwitch");
            return ASE_InvalidParameter;
        }
        if (numChannels != kHostChannels || bufferSize != kHostBufferFrames) {
            strcpy_s(last_error_, "B4B createBuffers requires 2 outputs x 512 frames");
            return ASE_InvalidParameter;
        }
        if (buffers_created_ || engine_.prepared()) {
            strcpy_s(last_error_, "B4B createBuffers rejected: already created");
            return ASE_InvalidMode;
        }
        ...
        for (long i = 0; i < numChannels; ++i) {
            ...
            if (bufferInfos[i].isInput != ASIOFalse ||
                ...
                strcpy_s(last_error_, "B4B requires unique output channels 0 and 1");
                return ASE_InvalidParameter;
            }
        ...
        strcpy_s(last_error_, "B4B buffers ready: B4A async PCM path preserved; host queries enabled");
        return ASE_OK;
    }
    ...
    ASIOError disposeBuffers() override {
        ...
        if (worker_thread_ || engine_.running()) {
            stop_result = stop();
            if (worker_thread_) {
                strcpy_s(last_error_, "B4B disposeBuffers WITHHELD: worker still not joined");
                return ASE_HWMalfunction;
            }
        }
        ...
        if (stop_result != ASE_OK) {
            strcpy_s(last_error_, "B4B disposeBuffers cleanup complete after worker failure");
            return stop_result;
        }

        strcpy_s(last_error_, "B4B disposeBuffers OK: worker absent; host + WaveRT buffers detached");
        return ASE_OK;
    }
...
private:
    static DWORD WINAPI worker_entry(LPVOID context) {
        ...
        std::printf("B4B worker START thread=%lu\n", GetCurrentThreadId());
        for (;;) {
            ...
            if (result == X4WaveRtProcessResult::StopRequested) {
                std::printf("B4B worker STOP requested thread=%lu\n", GetCurrentThreadId());
                break;
            }
            if (result == X4WaveRtProcessResult::Failed) {
                InterlockedExchange(&self->worker_failed_, 1);
                std::printf("B4B worker FAILED thread=%lu message=%s\n",
        ...
        std::printf("B4B worker EXIT thread=%lu\n", GetCurrentThreadId());
        return 0;
    }
    ...
    static bool notification_observer(
        ...
        std::printf(
            "B4B callback=%ld notificationIndex=%lu packet=%lu writePacket=%lu slot=%ld samplePosition=%lld timestampNs=%lld copy=%s thread=%lu\n",
            callback_ordinal,
    ...
    char last_error_[124] = "Stage B4B host query contract not initialized";
};
