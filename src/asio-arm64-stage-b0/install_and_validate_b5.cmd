...
if not "%KSERR%"=="0" (
  >>"%REPORT%" echo.
  >>"%REPORT%" echo B5 PRODUCT VALIDATION RESULT: BUSY_OR_INDETERMINATE code=%KSERR%
  echo X4 is BUSY or indeterminate. No lifecycle test was started.
  goto :busy
)

echo Capturing B5 public ASIO contract...
...
:busy
echo.
echo No unsafe pin open was attempted. Close X4 playback/change default output and run this same script once again.
echo Report: %REPORT%
pause
exit /b %KSERR%
