...
#error Stage B2 must be built for native Windows ARM64, not ARM64EC.
#endif
...
public:
    ...
    ASIOBool init(void* sysHandle) override {
        ...
        if (!preflight.device_found) {
            strcpy_s(last_error_, "Stage B2 init preflight FAILED: X4 msft_wave not found");
            return ASIOFalse;
        }
        ...
        if (!preflight.filter_opened) {
            sprintf_s(
                last_error_,
                sizeof(last_error_),
                "Stage B2 init preflight FAILED: filter open Win32=%lu",
        ...
        if (!preflight.local_ok || !preflight.global_ok) {
            sprintf_s(
                last_error_,
                sizeof(last_error_),
                "Stage B2 init preflight INDETERMINATE: C ok=%d err=%lu G ok=%d err=%lu",
        ...
        if (local_busy || global_busy) {
            sprintf_s(
                last_error_,
                sizeof(last_error_),
                "Stage B2 init preflight BUSY: C %lu/%lu G %lu/%lu; KsCreatePin SKIPPED",
        ...
        sprintf_s(
            last_error_,
            sizeof(last_error_),
            "Stage B2 init preflight FREE: C %lu/%lu G %lu/%lu; engine not prepared",
            preflight.local_current,
            preflight.local_possible,
            preflight.global_current,
            preflight.global_possible);
    ...
    ASIOError start() override {
        if (!initialized_free_ || !engine_.prepared()) {
            strcpy_s(last_error_, "Stage B2 start rejected: engine not prepared");
            return ASE_InvalidMode;
        }

        const bool ok = engine_.start_and_observe();
        strcpy_s(last_error_, engine_.last_message());
        return ok ? ASE_OK : ASE_HWMalfunction;
    }
    ...
    ASIOError stop() override {
        ...
        if (ok) {
            strcpy_s(last_error_, "Stage B2 stop OK: RUN->PAUSE->ACQUIRE->STOP");
            return ASE_OK;
        }
        strcpy_s(last_error_, engine_.last_message());
    ...
    ASIOError createBuffers(ASIOBufferInfo* bufferInfos, long numChannels, long bufferSize, ASIOCallbacks* callbacks) override {
        if (!initialized_free_) {
            strcpy_s(last_error_, "Stage B2 createBuffers rejected: init did not report FREE");
            return ASE_InvalidMode;
        }

        if (bufferInfos != nullptr || numChannels != 0 || bufferSize != 512 || callbacks != nullptr) {
            strcpy_s(last_error_, "Stage B2 diagnostic createBuffers requires (nullptr,0,512,nullptr)");
            return ASE_InvalidParameter;
        }

        if (engine_.prepared()) {
            strcpy_s(last_error_, "Stage B2 createBuffers rejected: engine already prepared");
            return ASE_InvalidMode;
        }
        ...
        case X4WaveRtPrepareResult::Ready:
            return ASE_OK;
        case X4WaveRtPrepareResult::Busy:
            return ASE_HWMalfunction;
        case X4WaveRtPrepareResult::Indeterminate:
            return ASE_HWMalfunction;
        case X4WaveRtPrepareResult::Failed:
        default:
            return ASE_HWMalfunction;
        }
    }
    ...
    ASIOError disposeBuffers() override {
        engine_.dispose();
        strcpy_s(last_error_, "Stage B2 disposeBuffers OK: event/pin/filter closed");
        return ASE_OK;
    }
...
private:
    ...
    char last_error_[124] = "Stage B2 COM/WaveRT diagnostic not initialized";
};
