...
#error Stage B3B WaveRT engine must be built for native Windows ARM64, not ARM64EC.
#endif
...
static_assert(kPacketBytes * kNotificationCount == kRequestedBufferBytes,
              "B3B packet geometry must exactly cover the WaveRT cyclic buffer");
...
X4WaveRtPrepareResult X4WaveRtEngine::prepare() {
    ...
    if (!find_x4_wave_path(path, sizeof(path) / sizeof(path[0]))) {
        strcpy_s(last_message_, "Stage B3B prepare FAILED: X4 msft_wave not found");
    ...
    if (filter_ == INVALID_HANDLE_VALUE) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B prepare FAILED: filter open Win32=%lu", GetLastError());
        return X4WaveRtPrepareResult::Failed;
    }
    ...
    if (!local_ok || !global_ok) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B PRE-PIN gate INDETERMINATE: C ok=%d err=%lu G ok=%d err=%lu; KsCreatePin SKIPPED",
    ...
    std::printf(
        "B3B PRE-PIN GATE: C %lu/%lu G %lu/%lu busy=%s\n",
        local_current, local_possible, global_current, global_possible,
        busy ? "YES" : "NO");

    if (busy) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B PRE-PIN gate BUSY: C %lu/%lu G %lu/%lu; KsCreatePin SKIPPED",
                  local_current, local_possible, global_current, global_possible);
        CloseHandle(filter_);
        filter_ = INVALID_HANDLE_VALUE;
    ...
    if (status != ERROR_SUCCESS || !created_pin || created_pin == INVALID_HANDLE_VALUE) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B KsCreatePin FAILED status=0x%08lX",
    ...
    if (!DeviceIoControl(
            ...
            &returned, nullptr)) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B BUFFER_WITH_NOTIFICATION FAILED Win32=%lu", GetLastError());
        dispose();
        return X4WaveRtPrepareResult::Failed;
    }
    ...
    if (!buffer.BufferAddress || buffer.ActualBufferSize != kRequestedBufferBytes) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B unexpected WaveRT buffer address=%p size=%lu",
    ...
    std::printf(
        "B3B WaveRT BufferAddress=%p ActualBufferSize=%lu CallMemoryBarrier=%d packetBytes=%lu\n",
    ...
    if (!event_) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B CreateEventW FAILED Win32=%lu", GetLastError());
        dispose();
        return X4WaveRtPrepareResult::Failed;
    }
    ...
    if (!DeviceIoControl(
        ...
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B REGISTER_NOTIFICATION_EVENT FAILED Win32=%lu", GetLastError());
        dispose();
        return X4WaveRtPrepareResult::Failed;
    }
    ...
    prepared_ = true;
    strcpy_s(last_message_, "Stage B3B WaveRT prepared: mapped 2x2048-byte packets; not RUN yet");
...
bool X4WaveRtEngine::write_interleaved_packet(
    ...
    if (!prepared_ || !buffer_address_ || actual_buffer_size_ != kRequestedBufferBytes) {
        strcpy_s(last_message_, "Stage B3B DMA copy FAILED: WaveRT buffer not prepared");
        return false;
    }
    if (!left || !right || frames != kPacketFrames) {
        strcpy_s(last_message_, "Stage B3B DMA copy FAILED: expected two non-null planar int16 x 512 buffers");
        return false;
    }
    ...
    if (byte_offset + kPacketBytes > actual_buffer_size_) {
        strcpy_s(last_message_, "Stage B3B DMA copy FAILED: packet offset out of range");
        return false;
    }
    ...
    std::printf(
        "B3B DMA writePacket=%lu slot=%lu frames=%lu nonzeroSamples=%lu\n",
...
bool X4WaveRtEngine::start_and_observe(
    ...
    if (!prepared_ || pin_ == INVALID_HANDLE_VALUE || !event_) {
        strcpy_s(last_message_, "Stage B3B start FAILED: engine not prepared");
        return false;
    }
    if (entered_run_) {
        strcpy_s(last_message_, "Stage B3B start FAILED: already RUN");
        return false;
    }
    ...
    if (!set_state(pin_, KSSTATE_ACQUIRE) ||
        ...
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B state transition FAILED Win32=%lu", GetLastError());
        return false;
    }
    entered_run_ = true;
    ...
    for (ULONG i = 0; i < kNotificationsPerRun; ++i) {
        ...
        if (wait != WAIT_OBJECT_0) {
            if (wait == WAIT_TIMEOUT) {
                strcpy_s(last_message_, "Stage B3B RUN FAILED: DMA notification timeout");
            } else {
                sprintf_s(last_message_, sizeof(last_message_),
                          "Stage B3B RUN FAILED: wait=%lu Win32=%lu", wait, GetLastError());
            }
            stop();
            return false;
        }
        ...
        if (!get_packet_count(pin_, &packet_count)) {
            sprintf_s(last_message_, sizeof(last_message_),
                      "Stage B3B PACKETCOUNT FAILED Win32=%lu", GetLastError());
            stop();
            return false;
        }
        ...
        if (!get_presentation_position(pin_, &position)) {
            sprintf_s(last_message_, sizeof(last_message_),
                      "Stage B3B PRESENTATION_POSITION FAILED Win32=%lu", GetLastError());
            stop();
            return false;
        }
        ...
        std::printf(
            "B3B notification=%lu packet=%lu writePacket=%lu slot=%lu samplePosition=%llu qpc=%llu\n",
            i + 1,
            packet_count,
            write_packet,
            write_slot,
        ...
        if (observer && !observer(observer_context, i, packet_count)) {
            if (last_message_[0] == '\0') {
                strcpy_s(last_message_, "Stage B3B observer/DMA copy FAILED");
            }
            stop();
            return false;
        }
    ...
    if (stats_.packet_discontinuities != 0 || stats_.position_regressions != 0) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B RUN quality FAILED: notifications=%lu packetDiscontinuities=%lu positionRegressions=%lu",
                  stats_.notifications,
                  stats_.packet_discontinuities,
                  stats_.position_regressions);
        stop();
    ...
    sprintf_s(last_message_, sizeof(last_message_),
              "Stage B3B RUN notifications=%lu dmaWrites=%lu dmaFrames=%lu nonzeroSamples=%lu lastWritePacket=%lu",
              stats_.notifications,
              stats_.hardware_buffer_writes,
              stats_.dma_frames_copied,
              stats_.dma_nonzero_samples,
              stats_.last_write_packet);
    return true;
}
...
bool X4WaveRtEngine::stop() {
    ...
    if (!ok) {
        sprintf_s(last_message_, sizeof(last_message_),
                  "Stage B3B stop FAILED Win32=%lu", GetLastError());
    }
    return ok;
}
...
void X4WaveRtEngine::dispose() {
    ...
    if (notification_registered_ && pin_ != INVALID_HANDLE_VALUE && event_) {
        ...
        if (DeviceIoControl(
                ...
                &notification, sizeof(notification),
                nullptr, 0, &returned, nullptr)) {
            std::printf("B3B unregister notification -> OK\n");
        } else {
            std::printf("B3B unregister notification -> FAIL Win32=%lu\n", GetLastError());
        }
    }
